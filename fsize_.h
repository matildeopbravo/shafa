...
for files not already opened (in this case the file is identified by
<filename>).
...
the original size in <*the_block_size>. If the original value of
<*the_block_size> is smaller than FSIZE_MIN_BLOCK_SIZE then
FSIZE_MIN_BLOCK_SIZE is used. Also, if the original value of <*the_block_size>
is bigger than FSIZE_MAX_BLOCK_SIZE then FSIZE_MAX_BLOCK_SIZE is used. If the
original value of
<*the_block_size> is equal to 0 (zero) then FSIZE_DEFAULT_BLOCK_SIZE is used.
Bear in mind that the last block may have a smaller size and its value is
returned in <*size_of_last_block>.

If n_blocks is the returned number of blocks then:
   total_file_size = n_blocks * <*the_block_size> + <*size_of_last_block>

Arguments:
  FILE *fp_in - a file pointer (if NULL, then the file is identified by
<filename>) unsigned char *filename - if NULL then file must be identified by
< *fp> unsigned long *the_block_size - this is used to pass the intended size
for each block and to receive the actual block size used long
*size_of_last_block - this is used to receive the size of the last block

This code is open source and free to use as long as the original author is
identified.
...
#define FSIZE_MAX_NUMBER_OF_BLOCKS                                             \
  4294967296 // Max number of blocks that can be returned = 2^32 blocks
#define FSIZE_MAX_SIZE_FSEEK                                                   \
  2147483648 // Max size permitted in fseek/ftell functions = 2 GBytes
#define FSIZE_ERROR_BLOCK_SIZE -1 // Error: Block size is larger than max value
#define FSIZE_ERROR_NUMBER_OF_BLOCKS                                           \
  -2 // Error: Number of Blocks exceeds max value permitted
...
size_t fsize(FILE *fp_in, char const *filename, size_t *the_block_size,
  ...
  *the_block_size = block_size;
  ...
  n_blocks = FSIZE_MAX_SIZE_FSEEK / block_size -
             1; // In reality fseek() can't handle FSIZE_MAX_SIZE_FSEEK of
                // 2GBytes, so let's use a smaller size
